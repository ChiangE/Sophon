...
class CIFAR9Dataset(Dataset):
    ...
    def __init__(self, root_dir, set_name='train', transform=None):
        ...
        self.labels = list(np.array(self.labels)[~index])
...
class aug_CIFAR9Dataset_cifar1(Dataset):
    ...
    def __init__(self, root_dir, set_name='train', transform=None):
        ...
        self.images = self.images[index]
        self.images = np.repeat(self.images, 10, axis=0)
        self.labels = list(np.array(self.labels)[index])
        self.labels = np.repeat(self.labels, 10, axis=0)
        meta_file_path = os.path.join(root_dir, 'batches.meta')
        with open(meta_file_path, 'rb') as f2:
            meta_data = pickle.load(f2, encoding='latin1')
            sub_class_name_list = meta_data['label_names']
...
class CIFAR9Dataset_cifar1(Dataset):
