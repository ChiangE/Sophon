'''
PLMS sample method
https://github.com/Stability-AI/stablediffusion/blob/main/ldm/models/diffusion/plms.py
'''
import os
import sys
...
class PLMSSampler(nn.Module):
    ...
    def __init__(self,
                 ...
                 clip_denoised=True):
        super(PLMSSampler, self).__init__()
    ...
    def get_x_prev_and_pred_x0(self, sample_images, pred_noise, index):
        device = sample_images.device
        b = sample_images.shape[0]
        # select parameters corresponding to the currently considered timestep
        a_t = (torch.ones((b, 1, 1, 1)) * self.ddim_alphas[index]).to(device)
        a_prev_t = (torch.ones(
            (b, 1, 1, 1)) * self.ddim_alphas_prev[index]).to(device)
        sigma_t = (torch.ones(
            (b, 1, 1, 1)) * self.ddim_sigmas[index]).to(device)
        sqrt_one_minus_at = (torch.ones(
            (b, 1, 1, 1)) * self.ddim_sqrt_one_minus_alphas[index]).to(device)
    ...
    def forward(self,
        ...
        for idx, time_step in enumerate(
                tqdm(all_steps,
                     desc='plms sampler time step',
                     total=len(all_steps))):
            ddim_step_index_before_flip = len(all_steps) - idx - 1

            time = (torch.ones((b, )) * time_step).long().to(device)
            time_next = (torch.ones(
                (b, )) * all_steps[min(idx + 1,
                                       len(all_steps) - 1)]).long().to(device)
            ...
            if len(history_eps) == 0:
                # Pseudo Improved Euler (2nd order)
                x_prev, _ = self.get_x_prev_and_pred_x0(
                    sample_images, pred_noise, ddim_step_index_before_flip)
                pred_noise_next = model(sample_images,
                                        time_next,
                                        class_label=class_label)
                pred_noise_prime = (pred_noise + pred_noise_next) / 2
            elif len(history_eps) == 1:
                # 2nd order Pseudo Linear Multistep (Adams-Bashforth)
                pred_noise_prime = (3 * pred_noise - history_eps[-1]) / 2
            elif len(history_eps) == 2:
                # 3nd order Pseudo Linear Multistep (Adams-Bashforth)
                pred_noise_prime = (23 * pred_noise - 16 * history_eps[-1] +
                                    5 * history_eps[-2]) / 12
            elif len(history_eps) >= 3:
                # 4nd order Pseudo Linear Multistep (Adams-Bashforth)
                pred_noise_prime = (55 * pred_noise - 59 * history_eps[-1] +
                                    37 * history_eps[-2] -
                                    9 * history_eps[-3]) / 24

            x_prev, _ = self.get_x_prev_and_pred_x0(
                sample_images, pred_noise_prime, ddim_step_index_before_flip)

            sample_images = x_prev

            history_eps.append(pred_noise)
...
if __name__ == '__main__':
    ...
    net.eval()
    plms_sampler = PLMSSampler(beta_schedule_mode='linear',
                               linear_beta_1=1e-4,
                               linear_beta_t=0.02,
                               cosine_s=0.008,
                               ddpm_t=1000,
                               ...
                               ddim_discr_method='uniform',
                               clip_denoised=True)
    all_step_images, last_step_images = plms_sampler(net, [4, 3, 32, 32],
                                                     class_label=None,
    ...
    for data in tqdm(test_loader):
        ...
        all_step_images, last_step_images = plms_sampler(
            net, [4, 3, 32, 32],
            class_label=labels,
            input_images=images,
            input_masks=None,
