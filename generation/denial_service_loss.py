...
def fast_adapt_multibatch(batches, learner, loss, shots, ways,trainer, device):
    ...
    for batch in batches:
        ...
        evaluation_error = loss(pred_noise_eva, torch.zeros_like(pred_noise_eva))
...
def main(
    ...
    for i in range(args.total_loop):
        ...
        for ml in range(args.ml_loop):
                ...
                # evaluation_error.clamp_(-40,0)# evaluation_error = torch.clip(evaluation_error,min=-40,max=0)  #这两个方法的剪裁会导致梯度后面变成0
...
if __name__ == '__main__':
    ...
    model, config, fid_model, target_trainloader, target_trainloader_fortest, target_testloader, original_testloder, trainer, train_criterion, sampler = main(args)
    ...
    if args.test_original == 'no':
        ...
        targettestloss = final_testfinetune_lr(f'final ours', test_model, fid_model, sampler, config, target_trainloader, target_trainloader_fortest, original_testloader, trainer, train_criterion, scheduler, finetune_optimizer, args.truly_finetune_epochs)
    wandb.log({"Finetune target test loss": targettestloss})
